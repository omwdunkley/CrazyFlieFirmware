...
void imu6Init(void) {
    ...
    cosPitch = cos(configblockGetCalibPitch() * DEG2RAD);
    sinPitch = sin(configblockGetCalibPitch() * DEG2RAD);
    cosRoll = cos(configblockGetCalibRoll() * DEG2RAD);
    sinRoll = sin(configblockGetCalibRoll() * DEG2RAD);
...
//    if (counter >= 9) {
//        temp_buffer = ms5611GetTemperature(MS5611_OSR_DEFAULT);
...
//    } else {
//        *temperature = temp_buffer;
//        *pressure = ms5611GetPressure(MS5611_OSR_DEFAULT);
//        *asl_baro = ms5611PressureToAltitude(pressure);
//    }
//    ++counter;
//}
