...
#define CONVERSION_TIME_MS   10 // used to be: 100 // conversion time in milliseconds
typedef struct {
    uint16_t psens;
    uint16_t off;
    uint16_t tcs;
...
bool ms5611SelfTest(void) {
    ...
    if (ms5611EvaluateSelfTest(MS5611_ST_PRESS_MIN, MS5611_ST_PRESS_MAX, pressure, "pressure") && ms5611EvaluateSelfTest(MS5611_ST_TEMP_MIN, MS5611_ST_TEMP_MAX, temperature, "temperature")) {
...
float ms5611PressureToAltitude(float* pressure/*, float* ground_pressure, float* ground_temp*/) {
    if (*pressure>0){
        ...
        return ((pow((1015.7 / *pressure), 1/5.257) - 1.0) * (25. + 273.15)) / 0.0065;
    } else {
        return 0;
    }
}
